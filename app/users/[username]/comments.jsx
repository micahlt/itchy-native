    ...
    const postComment = (content) => {
        ...
        if (!!reply?.id) {
            ...
            if (reply.parentID) {
                ...
                parentID = reply.parentID.includes("comments-") ? reply.parentID.split("comments-")[1] : reply.parentID;
            } else {
                // This is a reply to a top-level comment, use the comment's ID
                parentID = reply.id.includes("comments-") ? reply.id.split("comments-")[1] : reply.id;
            }
        }
                ...
                if (!!reply) {
                    setComments((prev) => prev.map(c => {
                        ...
                        if (c.id === reply.id) {
                            return {
                                ...c,
                                replies: [...c.replies, { author: { username: user.username, image: `https://cdn2.scratch.mit.edu/get_image/user/${user.id}_60x60.png` }, content, datetime_created: new Date(), id: `comments-${postedID}`, parentID: c.id, includesReplies: true, replies: [] }]
                            };
                        }
