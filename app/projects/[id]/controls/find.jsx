...
export default function FindControls() {
  ...
  const getMappings = () => {
    ...
    fetch(
      `https://itchy-controldb.vercel.app/api/controllermapping?projectId=${id}&date=${Date.now()}`
    )
  ...
  const makeNewMapping = () => {
    if (!!username) {
      ...
      ToastAndroid.show(
        "You must be logged in to create a new controller mapping.",
        ToastAndroid.LONG
      );
      router.push("/login");
    }
  ...
  const renderItem = (item) => {
    return (
      <Card
        style={{ padding: 10 }}
        onPress={() => setControllerForProject(item)}
      >
        <View
          style={{
            display: "flex",
        ...
        >
          <View style={{ flex: 1 }}>
            ...
            >
              ...
              >
                {item.controlOptions.primaryController || "Unknown"}
              </ItchyText>
            </ItchyText>
          ...
          <View style={{ flex: 1 }}>
            ...
            >
              ...
              >
                {item.controlOptions.secondaryController}
              </ItchyText>
            </ItchyText>
            {Object.keys(item.controls.secondary).map((key) => (
              ...
              >
                {key.toUpperCase()}:{" "}
                <ItchyText style={{ fontWeight: "bold", color: colors.text }}>
                  {controlOptionToFullName(item.controls.secondary[key])}
                </ItchyText>
              </ItchyText>
            ))}
          </View>
        ...
        >
          {item.controls.extra.length > 0 && (
            ...
            >
              EXTRAS:{" "}
              {item.controls.extra.map((key) => (
                <ItchyText
                  key={key}
                  style={{ fontWeight: "bold", color: colors.text }}
                >
                  {controlOptionToFullName(key)}{" "}
                </ItchyText>
              ))}{" "}
              —{" "}
            </ItchyText>
            ...
            <>
              ...
              >
                <TextInput
                  ...
                  inlineImageLeft={
                    isDark ? "search_24_white" : "search_24_black"
                  }
            ...
            query === "" ? (
              <Fragment key="empty-state">
                {!isLoading && (
                  <View style={{ padding: 20, alignItems: "center" }}>
                    ...
                    >
                      No controller mappings found for this project.
                    </ItchyText>
                    <TouchableOpacity
              ...
              >
                ...
                <ItchyText
                  style={{ color: colors.textSecondary, fontSize: 16 }}
                >
                  No controller mappings match that username.
                </ItchyText>
                <TouchableOpacity
                  onPress={makeNewMapping}
                  style={{ padding: 8 }}
