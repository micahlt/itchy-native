...
export default function Error() {
    ...
    return <View style={{ flex: 1, alignItems: "center", justifyContent: "start", backgroundColor: colors.background, paddingTop: 30 }}>
        ...
        <ItchyText style={{ color: colors.accent, fontSize: 24, fontWeight: "bold", textAlign: 'center', width: "80%", marginBottom: 10 }}>Whoops, the app is scratching its head!</ItchyText>
        {params.errorText ? <ItchyText style={{ color: colors.textSecondary }}>{params.errorText}</ItchyText> : <ItchyText style={{ color: colors.textSecondary }}>We ran into an issue. Please try again.</ItchyText>}
        <View style={{ borderRadius: 10, overflow: 'hidden', backgroundColor: colors.accent, margin: 10, elevation: 2 }}>
            <Pressable onPress={() => router.back()} style={{ padding: 10, paddingHorizontal: 15 }} android_ripple={{ color: colors.ripple, borderless: false, foreground: true }}>
                <ItchyText style={{ color: colors.text, textAlign: "center", fontWeight: "bold" }}>Go back</ItchyText>
            </Pressable>
        </View>
    </View>
};