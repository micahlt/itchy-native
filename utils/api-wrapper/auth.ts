...
const APIAuth = {
  login: async (user: string, pass: string): Promise<AuthSession> => {
  ...
  logout: async (cookie?: string): Promise<void> => {
    ...
    const csrfToken =
      /scratchcsrftoken=(.*?);/gm.exec(setCookie || "")?.[1] || "";
  ...
  getSession: async (existingCookies: string = ""): Promise<AuthSession> => {
    const sessionFetch = await fetch("https://scratch.mit.edu/session?", {
      method: "GET",
      headers: {
