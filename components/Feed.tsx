...
export default memo(function Feed({ username, style }: FeedProps) {
  const { colors, dimensions } = useTheme();
  const [token] = useMMKVString("token");
  const router = useRouter();

  // SWR data fetching with shared key for external refresh
  const { data: rawFeed, isLoading } = useSWR<UserActivity[]>(
    ...
    <>
      ...
      <SquircleView
        style={{
          ...
          boxShadow:
            "0px 8px 6px 0px #ffffff22 inset, 0px 2px 0px 0px #FFFFFF33 inset",
        }}
      >
        <FlatList
          data={feed}
          ...
          keyExtractor={(item, index) =>
            item.id?.toString() || `feed-item-${index}`
          }
