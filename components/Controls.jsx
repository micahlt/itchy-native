...
import { FullscreenContext } from "./Controls/FullscreenContext";
...
export default function Controls({
  ...
  const animatedContainerStyle = useAnimatedStyle(() => {
    const currentExpandedHeight = isAnimating.value
      ? lockedExpandedHeight.value
    ...
    return {
      height:
        animatedHeight.value === 1 ? Math.max(totalHeight, 280) : totalHeight, // Ensure minimum height when expanded
  ...
  const animatedSquircleStyle = useAnimatedStyle(() => {
    ...
    const height = interpolate(
      ...
      [
        ...
        Math.max(
          280,
          isAnimating.value ? lockedExpandedHeight.value : expandedHeight.value,
        ),
      ], // Ensure adequate initial height
      Extrapolate.CLAMP,
    );
    ...
    return {
      ...
      overflow: "hidden",
      boxShadow:
        "0px 10px 20px 0px #FFFFFF40 inset, 0px 2px 1px 0px #FFFFFF40 inset, 0px -5px 0px 0px #0000001A inset, 0px 2px 5px 2px rgba(76, 148, 255, 0.275), 0px 8px 6px 0px #FFFFFF33 inset, 0px 4px 0px 0px #FFFFFF4D inset",
      width: "100%",
  ...
  const animatedLinearGradientStyle = useAnimatedStyle(() => {
    const height = interpolate(
      ...
      ], // Use full height, no subtraction
      Extrapolate.CLAMP,
    );
  ...
  useEffect(() => {
    fetch(
      `https://itchy-controldb.vercel.app/api/controllermapping?projectId=${projectId}&mappingId=${localControllerMappings ? (localControllerMappings[projectId] ? localControllerMappings[projectId] : "") : ""}`,
    )
        ...
        <View
          ...
          onLayout={(event) => {
            ...
            if (
              ...
              Math.abs(height - expandedHeight.value) > 5
            ) {
                  ...
                  <View
                    style={{
                      ...
                      paddingHorizontal: Math.max(
                        10,
                        containerDimensions.width * 0.02,
                      ),
                    }}
                  >
                    {currentMapping.controlOptions.showPrimaryController && (
              ...
              <Animated.View style={animatedLinearGradientStyle}>
                ...
                >
                  {currentMapping?.controlOptions ? (
                    ...
                    <ItchyText style={{ color: colors.text, padding: 20 }}>
                      Loading controls...
                    </ItchyText>
                  )}
                </LinearGradient>
