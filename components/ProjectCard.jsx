        ...
        return (
            <Pressable style={{
                ...
                <View style={{
                    ...
                    {project.creator && <TouchableOpacity onPress={() => router.push(`/user/${project.creator || project.author?.username}/profile`)}>
                        <Text style={{ color: colors.accent, padding: 10, paddingTop: 2, fontSize: 12 }} numberOfLines={1}>{project.creator}</Text>
                    </TouchableOpacity>}
                    {project.author?.username && <Chip.Image imageURL={project.author.profile.images["90x90"]} text={project.author.username} onPress={() => router.push(`/user/${project.author.username}/profile`)} textStyle={{ fontSize: 12, fontWeight: "bold", color: colors.accent }} style={{ marginRight: "auto", marginBottom: 8, marginLeft: 8, marginTop: 4, backgroundColor: colors.backgroundTertiary }} />}
